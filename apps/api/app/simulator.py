...
class DeterministicSimulator:
    ...
    def create_run(self, config: ScenarioConfig | None = None) -> SimulationRun:
        ...
        run = SimulationRun(id=f"run-{abs(hash((scenario.seed, scenario.start_time, scenario.scenario_version)) % 100000)}", config=scenario, clock=scenario.start_time)
    ...
    def advance(self, run_id: str, duration_hours: int = 8) -> list[SimulationEvent]:
        ...
        for index in range(max(1, duration_hours // 2)):
            ...
            events.append(event)
        run.clock = (base_time + timedelta(hours=duration_hours)).isoformat().replace('+00:00', 'Z')
    ...
    def replay(self, run_id: str) -> list[SimulationEvent]:
        run = self._runs[run_id]
        return [event for event in run.events]
